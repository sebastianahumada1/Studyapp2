...
import { useCreateTopic } from '@/hooks/useKnowledgeHub';
...
export function CreateTopicModal({ routeId, onClose }: CreateTopicModalProps) {
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        <h2 className="text-2xl font-bold text-white mb-6 pr-10">Agregar Tema</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-white text-sm font-medium mb-2">Nombre del Tema</label>
            <input
              type="text"
              value={name}
          ...
          <div className="flex gap-4 justify-end pt-4">
            ...
            >
              {createTopic.isPending ? 'Creando...' : 'Crear Tema'}
            </button>
          </div>
        </form>
      </div>
